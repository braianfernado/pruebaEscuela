...
#region Peticiones Api Rest Profesor
...
#region Peticiones Api Rest EstudiantesMateria
app.MapGet("/EstudiantesMateria/lista", async (
    IEstudianteService _estudianteService,
    IMapper _mapper

    ) =>
{
    var listaEstudiantesMateri = await _estudianteService.GetList();
    var listaEstudiantesMateriaDTO = _mapper.Map<List<ProfesorDTOs>>(listaEstudiantesMateri);


    if (listaEstudiantesMateriaDTO.Count > 0)
        return Results.Ok(listaEstudiantesMateriaDTO);
    else
        return Results.NotFound();
});
